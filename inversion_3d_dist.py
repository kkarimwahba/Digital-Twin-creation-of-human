...
from training.inversion import inversion_3d
...
def subprocess_fn(rank, c, temp_dir):
    ...
    if c.inference_vis:
        inversion_3d.inversion(rank=rank, **c)
...
@click.option('--inference_for_pck_eval', help='whther we run infernce', metavar='BOOL', type=bool, default=False, show_default=True)
@click.option('--inference_for_depth_eval', help='whther we run infernce', metavar='BOOL', type=bool, default=False, show_default=True)
...
@click.option('--num_steps_pti', help='Number of optimization steps for pivot tuning', type=int, default=200, show_default=True)
@click.option('--save_video', help='Save an mp4 video of optimization progress', type=bool, default=True, show_default=True)
@click.option('--save_multiview', help='Save an mp4 video of optimization progress', type=bool, default=True, show_default=True)
@click.option('--save_animation', help='Save an mp4 video of optimization progress', type=bool, default=True, show_default=True)
@click.option('--use_normal_map', help='Use normal map for inversion', type=bool, default=True, show_default=True)
...
def main(**kwargs):
    ...
    if opts.inference_vis:
        c.inference_for_pck_eval = opts.inference_for_pck_eval
        c.inference_to_generate_textured_mesh = opts.inference_to_generate_textured_mesh
        c.inference_for_depth_eval = opts.inference_for_depth_eval
        ...
        c.num_steps_pti = opts.num_steps_pti
        c.save_video = opts.save_video
        c.save_animation = opts.save_animation
        c.save_multiview = opts.save_multiview
        c.use_normal_map = opts.use_normal_map
